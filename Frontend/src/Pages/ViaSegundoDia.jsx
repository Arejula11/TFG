...
export const ViaSegundoDia = () => {
  ...
  // Si no hay datos de segundoDia, muestra mensaje
  if (!via.segundoDia || !Array.isArray(via.segundoDia)) {
    return (
      <div className="w-full h-full flex flex-col items-center align-middle justify-center">
        <NavBar id={id} />
        <Header Title={"Segundo día"} />
        <h1 className="text-red-600 text-3xl font-semibold">No hay datos de segundoDia</h1>
      </div>
    );
  }

  // Si los labels vienen en el objeto via.segundoDia, por ejemplo via.segundoDia[i].label
      ...
      <div className="mt-5 grid grid-cols-1 gap-7 ml-40 ">
        {via.segundoDia.map((item, idx) => (
          <div key={item.id} className="grid grid-cols-2 p-2 gap-x-20">
            ...
            {item.tipo === "booleano" && (
              <RadioGroup
                name={`segundoDia${item.id}`}
                orientation="horizontal"
                value={item.estado}
                onValueChange={value => {
                  setVia(prevVia => ({
                    ...prevVia,
                    segundoDia: prevVia.segundoDia.map((q, i) =>
