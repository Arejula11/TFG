...
export const ViaPrimerDia = () => {
  ...
  // Si no hay datos de primerDia, muestra mensaje
  if (!via.primerDia || !Array.isArray(via.primerDia)) {
    return (
      <div className="w-full h-full flex flex-col items-center align-middle justify-center">
        <NavBar id={id} />
        <Header Title={"Primer día"} />
        <h1 className="text-red-600 text-3xl font-semibold">No hay datos de primerDia</h1>
      </div>
    );
  }

  // Si los labels vienen en el objeto via.primerDia, por ejemplo via.primerDia[i].label
      ...
      <div className="mt-5 grid grid-cols-1 gap-7 ml-40 ">
        {via.primerDia.map((item, idx) => (
          <div key={item.id} className="grid grid-cols-2 p-2 gap-x-20">
            ...
            {item.tipo === "booleano" && (
              <RadioGroup
                name={`primerDia${item.id}`}
                orientation="horizontal"
                value={item.estado}
                onValueChange={value => {
                  setVia(prevVia => ({
                    ...prevVia,
                    primerDia: prevVia.primerDia.map((q, i) =>
