...
class ProbabilisticTimePetriNet:
    ...
    def build_lola(self):
        """
        Exporta la red de Petri al formato LoLA (estructura básica).
        return: string con el contenido en formato LoLA
        """
        # Declarar plazas
        lola = "PLACE " + ", ".join([p.name for p in self.places]) + ";\n\n"
        # Marcado inicial
        initial = []
        for p in self.places:
            if p.initial_marking > 0:
                if p.initial_marking == 1:
                    initial.append(p.name)
                else:
                    initial.append(f"{p.initial_marking}*{p.name}")
        lola += "MARKING " + ", ".join(initial) + ";\n\n"
        # Transiciones
        transition_counter = 0
        for t in self.transitions:
            for i, (outcome, prob) in enumerate(zip(t.post[0].outcomes, t.post[0].probabilities)):
                transition_name = f"T{transition_counter}"
                transition_counter += 1
                lola += f"TRANSITION {transition_name}\n"

                # CONSUME
                pre = []
                for place, weight in t.pre.items():
                    pre.append(f"{weight}*{place.name}" if weight > 1 else place.name)
                lola += "  CONSUME " + ", ".join(pre) + ";\n"

                # PRODUCE
                post = []
                for place, weight in outcome.items():
                    post.append(f"{weight}*{place.name}" if weight > 1 else place.name)
                lola += "  PRODUCE " + ", ".join(post) + ";\n\n"
...
class Transition:
    ...
    def get_fire_duration(self):
        ...
        if self.time_function_type == "exponential":
            return random.expovariate(self.time_function["lambd"])
    ...
    def get_expected_duration(self):
        ...
        if self.time_function_type == "exponential":
            return 1/self.time_function["lambd"]
        if self.time_function_type == "gamma":
