...
# Modified from the original code to be more modular and easier to read
# Miguel Aréjula 2025-06-10
...
def main(csv_path):
    global filename, name, act_row, res_row
    filename = csv_path.split("/")[-1].split(".")[0]
    name = {
        "via": "vias_clinicas"
    }.get(filename, filename)
    act_row = {"via": 1}.get(filename, 1)
    res_row = {"via": 3}.get(filename, 3)
    ...
    with open(csv_path) as csvfile:
        ...
        for row in spamreader:
            if filename == "sepsis":
                ...
                time += random.lognormvariate(1, 1)
            else:
                time += random.randint(1, max_activity_time)
            # Only start a new sequence if NHC is present (not empty after stripping)
            if row[0].strip():
                nhc = row[0].strip()
                #print("New sequence for NHC", nhc, "at", time)
                if sequence:
    ...
    if filename != "sepsis":
        ...
        # print("Number of sequences:", len(hcu_database))
        # print("Sequences:", hcu_database)
    else:
    ...
    if use_resources:
        ...
        with open("PetriNet" + '.xml', 'w') as f:
            f.write(PTPN.build_xml())
        PTPN.build_graphviz_png(path=name)
        with open("PetriNet" + '.lola', 'w') as f:
            f.write(PTPN.build_lola())
        # PTPN.build_graphviz_png(path=name)
        with open("PetriNet" + '.lola') as f:
            lola_content = f.read()
    ...
    import os
    import glob
    files = glob.glob("lognorm_dist/*.png")
    for f in files:
        os.remove(f)
    os.remove("PTPN_article_fig1.png")
    os.remove("PTPN_article_fig1")
    os.remove("shortest_path.png")
    os.remove("shortest_path")
